...
def extract_hidden_pixels(image, width_visible, height_visible, pixel_count):
    ...
    Remark the pixels of the hidden image should be 2 times smaller than number of pixels in visible image
    ...
    for col in range(width_visible):
        for row in range(height_visible):
            if row == 0 and col == 0:
                continue
            red, green, blue, _ = image[col, row]
...
def reconstruct_image(image_pixels, width, height):
    """
    Reconstructs the hidden image using the extracted string of pixel binary values.
...
def decode_image(image_path):
    ...
    Args:
        image_path:    An RGB image to recover a hidden image from
    ...
    red, green, blue, _ = image_copy[0, 0]
    r_binary, g_binary, b_binary = rgb_to_binary(red, green, blue)
